/**
 * Nigeria: 36 states + the FCT, and all 774 LGAs.
 *
 * This replaces `country-state-city` (8.7MB). That package was lazy-loaded, but
 * lazy-loading 8.7MB on a 2G connection still means onboarding never finishes —
 * which is exactly where a new tailor gives up. This file is a few KB.
 *
 * LGAs, not cities, are the unit here on purpose: an LGA is how Nigerians
 * actually state where they are, and it is the granularity every official form
 * uses. Falling back to a scraped city list would be both larger and less
...
export function getLgasOfState(state) {
